...
if plotresults
    ...
    errorbar(ncY, CV_lambda - CV_0_inf, CV_lambda_SE, 'o', 'LineWidth', LineWidth, 'Color', colors(j,:))
    ...
    errorbar(ncY, CV_0 - CV_lambda_inf, CV_0_SE, 'o', 'LineWidth', LineWidth, 'Color', colors(j,:))
    xlabel('Domain width N_y (cells)')
    ...
    for j = 1:numel(readouts)
        ...
        subplot(1,3,3)
        hold on
        mdl = fitnlm(ncY(fitrange), sigma_x(fitrange,j) / diameter, @(p,x) p(1)./sqrt(x)+p(2), [1 0]);
        pe_inf = mdl.Coefficients.Estimate(2);
        plot(ncY, feval(mdl, ncY) - pe_inf, '-', 'HandleVisibility', 'off', 'LineWidth', LineWidth, 'Color', colors(j,:))
        errorbar(ncY, sigma_x(:,j) / diameter - pe_inf, sigma_x_SE(:,j) / diameter, 'o', 'DisplayName', name, 'LineWidth', LineWidth, 'Color', colors(j,:))
        xlabel('Domain width N_y (cells)')
